...
"""
CuTeDSL Dense GEMM with Cluster Support:
...
def kernel(
    ...
    if warp_idx == 0:
        ...
        # [PAIR-UMMA] manualy prefetch
        prefetch_k_tile_cnt = cutlass.min(ab_stages - 2, num_k_tiles)
        ...
        for k_tile_idx in cutlass.range(num_k_tiles):
            # Issue TMA loads
            if k_tile_idx < num_k_tiles - prefetch_k_tile_cnt:
                ab_empty = ab_producer.acquire_and_advance(
            ...
            if k_tile_idx + 1 < num_k_tiles - prefetch_k_tile_cnt:
                peek_ab_empty_status = ab_producer.try_acquire()
            if k_tile_idx + 1 < num_k_tiles and is_leader_cta:
                peek_ab_full_status = ab_consumer.try_wait()
...
def host_function(a: cute.Tensor, b: cute.Tensor, c: cute.Tensor, stream):
    ...
    # [PAIR-UMMA] Compute epi_tile using sm100_utils
    ...
    # Launch the kernel with cluster support
    cluster_shape_mnl = (*cluster_shape_mn, 1)
    # [PAIR-UMMA] grid size need to consider the 2cta influece on m dimension, 50% errors -> 0%
...
def run_dense_gemm(
    ...
    def create_tensors(l, m, n, k, a_major, b_major, c_major, ab_dtype, c_dtype):
        ...
        # a_torch_cpu = cutlass_torch.matrix(l, m, k, a_major == "m", ab_dtype) # no prepend l
