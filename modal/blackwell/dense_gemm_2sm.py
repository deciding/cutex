...
def run(
    ...
    **kwargs,
    ...
    M, N, K, L = mnkl
    use_tma_multicast = cluster_shape_mn[0] > 1 or cluster_shape_mn[1] > 1
    
    # Map cutlass dtypes to torch dtypes
    def to_torch_dtype(d):
        if d == cutlass.Float16: return torch.float16
        if d == cutlass.Float32: return torch.float32
        if d == cutlass.BFloat16: return torch.bfloat16
        return torch.float16

    torch_dtypes = (to_torch_dtype(ab_dtype), to_torch_dtype(ab_dtype), to_torch_dtype(c_dtype))
    
    # Map majors
    def to_major(m):
        if m.lower() == "k": return cute.nvgpu.tcgen05.OperandMajorMode.K
        return cute.nvgpu.tcgen05.OperandMajorMode.MN
    
    majors = (to_major(a_major), to_major(b_major), to_major(c_major))

    A_torch, B_torch, D_torch, A_cute, B_cute, D_cute = get_gemm_tensors(
        M, N, K, majors, torch_dtypes
    )
    ...
    if not skip_ref_check:
        ...
        torch.testing.assert_close(D_torch.float(), ref, atol=tolerance, rtol=tolerance)
    
    return avg_time_ms * 1000 # Return microseconds for consistency
